    ...
    return (
      <div className="flex h-screen bg-gray-50 text-gray-900">
        ...
        <aside className={`transition-all duration-300 ease-in-out ${sidebarOpen ? 'w-80' : 'w-16'} bg-white/80 backdrop-blur-xl flex flex-col h-full shadow-sm relative`}>
          ...
          {/* Notebooks List (replaces placeholder chat history) */}
              ...
              {availableNotebooks.length === 0 ? (
                sidebarOpen && (
                  <li className="px-3 py-2 text-xs text-gray-400">
                    No notebooks yet – create one with "New Chat"
                  </li>
                )
              ) : (
                availableNotebooks.map((notebook) => (
                  <li
                    key={notebook.id}
                    onClick={() => switchToNotebook(notebook)}
                    className={`flex items-center gap-3 px-3 py-3 rounded-xl cursor-pointer hover:bg-gray-50 transition-all duration-200 ${
                      sidebarOpen ? '' : 'justify-center'
                    } ${
                      notebook.id === currentNotebookId ? 'bg-gray-100' : ''
                    }`}
                  >
                    {sidebarOpen && (
